...
typedef struct {
    ...
    uint32_t fwVer;
    uint32_t fwSize;
    uint32_t auth_state;              ///< Device Authentication state; Interpreted as "whether device is authenticated or not?"; Indicated by ENUM device_auth_state (32-bit value)
    uint32_t first_boot_on_update;    ///< 0 - not first boot after update, 1 - first boot after update
    uint8_t flashHash[32];
}upgradeInfo_t;

#define READ_JOYSTICK(PinMask)  (READ_BIT(BSP_JOYSTICK_GPIO_PORT->IDR, PinMask) == (PinMask))
...
#define BSP_GET_IRQ_STATUS()		((READ_BIT(BSP_PN532_INT_PORT->IDR, BSP_PN532_INT_PIN) == (BSP_PN532_INT_PIN)) ? 1UL : 0UL)//HAL_GPIO_ReadPin(BSP_PN532_INT_PORT, BSP_PN532_INT_PIN)
